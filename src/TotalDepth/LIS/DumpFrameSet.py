...
def dump_frame_sets(fp, keep_going, summary_only, channels):
    """Dump the frame values to stdout.

    summary_only is a bool to emit a summary only, if false all the data and the summary is written out.
        ...
        if aLp.logPass.totalFrames == 0:
            ...
            if not summary_only:
                ...
                if frame_set.isIndirectX:
                    headers.append('XAXIS [{!r:s}]'.format(frame_set.xAxisDecl.depthUnits))
                indexes = []
                if len(channels):
                    for i, (m, u) in enumerate(aLp.logPass.genFrameSetHeadings()):
                        if m in channels:
                            headers.append('{!r:s} [{!r:s}]'.format(m, u))
                            indexes.append(i)
                else:
                    headers.extend(['{!r:s} [{!r:s}]'.format(m, u) for m, u in aLp.logPass.genFrameSetHeadings()])
                ...
                for frIdx in range(frame_set.number_of_frames):
                    ...
                    if len(indexes):
                        ...
                        print('\t'.join(['%g' % v for v in frame_set.frame(frIdx)]))
...
Reads a LIS file and writes out tab separated values of each frame."""
    ...
    option_parser.add_option("-s", "--summary", action="store_true", dest="summary", default=False,
    ...
    if len(args) != 1:
        ...
        return 1
    dump_frame_sets(args[0], opts.keepGoing, opts.summary, set([v.encode('ascii') for v in opts.channels]))
