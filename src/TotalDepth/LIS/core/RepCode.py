...
def writeBytes68(v):
    ...
    try:
        ...
        raise ExceptionRepCodeWrite(
            'RepCode.writeBytes68(): value={:s} w={:s} error: {:s}'.format(str(v), str(w), str(err)))
...
def writeBytes(v, r):
    ...
    try:
        ...
        raise ExceptionRepCodeWrite(
            'RepCode.writeBytes(): value={:s} rc={:s} error: {:s}'.format(str(v), str(r), str(err)))
    except KeyError:
