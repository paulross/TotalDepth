...
from TotalDepth.RP66.core import LogiData

def scanFile(fp, keepGoing, theS=sys.stdout):
    try:
        myLd = LogiData.LogiDataRead(fp)
    except LogiData.ExceptionLogiData as err:
        print('Can not open file, error: %s' % str(err))
        return
    theS.write('File:\n')
    theS.write('{:s}\n'.format(myLd.strOfFile))
    numLd = 0
    while True:
        numLd += 1
        myStr = '{:s}'.format(myLd)
        l = myLd.skipToNextRecord()
        if l < 0:
            theS.write('EOF\n')
            break
#        theS.write('{:s} data length={:d}\n'.format(myStr, l))
    theS.write('Logical data segments count: %d\n' % numLd)
