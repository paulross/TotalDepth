...
* A physical file (typically a file on disk) consists of a Storage Unit Label followed by any number of *Logical Files*.
* *Logical Files* consist of any number of :term:`Explicitly Formatted Logical Record` (EFLRs) and, optionally, a single *Log Pass*. Logical Files are independent from each other.
